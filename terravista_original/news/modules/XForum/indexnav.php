...
else //- Creation de l'index pour un gid particulier -
{
...
while($group = mysql_fetch_array($queryg)) 
{
  ...
  if($catsonly != "on" || $gid) 
  {
	...
	while($forum = mysql_fetch_array($query)) 
	{
	  ...
	  $strtemp .= Ligneforum($forum[lastpost], $timeoffset, $forum[moderator], $lastvisit2, $hideprivate, $status, $forum[private], $forum[posts], $forum[threads], $altbg1, $altbg2, $forum[name], $forum[fid], $forum[description], $timecode, $dateformat, $thisuser, $forum[userlist], $res3);
	}
  }

}
 echo $strtemp;
 mysql_free_result($queryg);
