...
function navbar($pollID, $title, $thold, $mode, $order) {
    global $user, $bgcolor1, $bgcolor2, $textcolor1, $textcolor2, $anonpost, $pollcomm, $prefix;
    $query = mysql_query("select pollID FROM $prefix"._pollcomments." where pollID=$pollID");
    if(!$query) $count = 0; else $count = mysql_num_rows($query);
    $result = mysql_query("select pollTitle from $prefix"._poll_desc." where pollID=$pollID");
	...
	if ($anonpost==1 OR is_admin($admin) OR is_user($user)) {
	    echo "</font></td><td bgcolor=\"$bgcolor1\"><font size=\"2\"><form action=\"pollcomments.php\" method=\"post\">"
...
function DisplayKids ($tid, $mode, $order=0, $thold=0, $level=0, $dummy=0, $tblwidth=99) {
	...
	$comments = 0;
	cookiedecode($user);
	$result = mysql_query("select tid, pid, pollID, date, name, email, url, host_name, subject, comment, score, reason from $prefix"._pollcomments." where pid = $tid order by date, tid");
	if ($mode == 'nested') {
		...
		while (list($r_tid, $r_pid, $r_pollID, $r_date, $r_name, $r_email, $r_url, $r_host_name, $r_subject, $r_comment, $r_score, $r_reason) = mysql_fetch_row($result)) {
			...
			// enter hex color between first two appostrophe for second alt bgcolor
				...
				if ($anonpost==1 OR is_admin($admin) OR is_user($user)) {
				    echo "<font size=2 color=\"$bgcolor2\"> [ <a href=\"pollcomments.php?op=Reply&pid=$r_tid&pollID=$r_pollID&mode=$mode&order=$order&thold=$thold\">"._REPLY."</a>";
				}
		...
		while (list($r_tid, $r_pid, $r_pollID, $r_date, $r_name, $r_email, $r_url, $r_host_name, $r_subject, $r_comment, $r_score, $r_reason) = mysql_fetch_row($result)) {
			if($r_score >= $thold) {
				...
				echo "</td></tr></table><br><p><font size=2 color=\"$bgcolor2\"> [ <a href=\"pollcomments.php?op=Reply&amp;pid=$r_tid&amp;pollID=$r_pollID&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._REPLY."</a>";
				modtwo($r_tid, $r_score, $r_reason);
				echo " ]</font><p>";
		...
		while (list($r_tid, $r_pid, $r_pollID, $r_date, $r_name, $r_email, $r_url, $r_host_name, $r_subject, $r_comment, $r_score, $r_reason) = mysql_fetch_row($result)) {
			if($r_score >= $thold) {
				...
				if (!eregi("[a-z0-9]",$r_subject)) $r_subject = "["._NOSUBJECT."]";
				formatTimestamp($r_date);
				echo "<li><font size=\"2\"><a href=\"pollcomments.php?op=showreply&tid=$r_tid&pollID=$r_pollID&pid=$r_pid&mode=$mode&order=$order&thold=$thold#$r_tid\">$r_subject</a> "._BY." $r_name "._ON." $datetime</font><br>";
...
function DisplayBabies ($tid, $level=0, $dummy=0) {
	...
	while (list($r_tid, $r_pid, $r_pollID, $r_date, $r_name, $r_email, $r_url, $r_host_name, $r_subject, $r_comment, $r_score, $r_reason) = mysql_fetch_row($result))
	{
		...
		formatTimestamp($r_date);
		echo "<a href=\"pollcomments.php?op=showreply&tid=$r_tid&mode=$mode&order=$order&thold=$thold\">$r_subject</a><font size=\"2\"> "._BY." $r_name "._ON." $datetime<br>";
...
function DisplayTopic ($pollID, $pid=0, $tid=0, $mode="thread", $order=0, $thold=0, $level=0, $nokids=0) {
    ...
    // with this you can see who is flaming you... ha-ha-ha
		...
		if(is_admin($admin)) {
		    $result= mysql_query("select host_name from $prefix"._pollcomments." where tid='$tid'");
		...
		echo "</td></tr></table><br><p>";
		if ($anonpost==1 OR is_admin($admin) OR is_user($user)) {
		    echo "<font size=\"2\"> [ <a href=\"pollcomments.php?op=Reply&amp;pid=$tid&amp;pollID=$pollID&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">"._REPLY."</a>";
		} else {
		    echo "[ "._NOANONCOMMENTS." ";
		}
		if ($pid != 0) {
			list($erin) = mysql_fetch_row(mysql_query("select pid from $prefix"._pollcomments." where tid=$pid"));
...
function singlecomment($tid, $pollID, $mode, $order, $thold) {
	//include("mainfile.php");
	include("header.php");
	global $user, $cookie, $datetime, $bgcolor1, $bgcolor2, $bgcolor3, $anonpost, $admin, $anonymous, $prefix;
	$deekayen = mysql_query("select date, name, email, url, subject, comment, score, reason from $prefix"._pollcomments." where tid=$tid and pollID=$pollID");
	...
	echo "<table width=99% border=0><tr bgcolor=\"$bgcolor1\"><td width=500>";
	formatTimestamp($date);
	if($email) echo "<p><b>$subject</b> <font size=2>("._SCORE." $score)<br>"._BY." <a href=\"mailto:$email\"><font color=\"$bgcolor2\">$name</font></a> <font size=2><b>($email)</b></font> "._ON." $datetime";
	else echo "<p><b>$subject</b> <font size=2>("._SCORE." $score)<br>"._BY." $name "._ON." $datetime";
	echo "</td></tr><tr><td>$comment</td></tr></table><br><p><font size=2 color=\"$bgcolor2\"> [ <a href=\"pollcomments.php?op=Reply&pid=$tid&pollID=$pollID&mode=$mode&order=$order&thold=$thold\">"._REPLY."</a> | <a href=\"pollBooth.php?pollID=$pollID\">"._ROOT."</a>";
	modtwo($tid, $score, $reason);
	echo " ]";
	modthree($pollID, $mode, $order, $thold);
	include("footer.php");
...
function reply ($pid, $pollID, $mode, $order, $thold) {
	//include("mainfile.php");
	include("header.php");
	global $user, $cookie, $datetime, $bgcolor1, $bgcolor2, $bgcolor3, $AllowableHTML, $anonymous, $prefix;
	if($pid!=0) {
		list($date, $name, $email, $url, $subject, $comment, $score) = mysql_fetch_row(mysql_query("select date, name, email, url, subject, comment, score from $prefix"._pollcomments." where tid=$pid"));
	} else {
		list($subject) = mysql_fetch_row(mysql_query("select pollTitle FROM $prefix"._poll_desc." where pollID=$pollID"));
	}
	if($comment == "") {
	...
	formatTimestamp($date);
	OpenTable();
	echo "<center><font size=\"4\"><b>"._SURVEYCOM."</b></font></center>";
	CloseTable();
	echo "<br>";
	OpenTable();
	echo "<center><font size=\"2\"><b>$subject</b></center><br>";
	if ($comment == "") {
	...
	if($pid == 0) {
		...
		list($subject) = mysql_fetch_row(mysql_query("select subject from $prefix"._pollcomments." where tid=$pid"));
	}
	echo "<br>";
	OpenTable();
	...
	if (is_user($user)) {
		cookiedecode($user);
		echo "<font size=\"2\"><a href=\"user.php\">$cookie[1]</a> [ <a href=\"user.php?op=logout\">"._LOGOUT."</a> ]</font>";
	} else {
		echo "<font size=\"2\">$anonymous</font>";
		$xanonpost=1;
	}
	echo "<br><br><font size=\"3\"><B>"._SUBJECT.":</B></FONT><BR>";
	...
	echo "<br><br><font size=\"3\"><B>"._UCOMMENT.":</B></FONT><BR>"
...
function replyPreview ($pid, $pollID, $subject, $comment, $xanonpost, $mode, $order, $thold, $posttype) {
	...
	OpenTable();
	echo "<center><font size=\"4\"><b>"._SURVEYCOMPRE."</b></font></center>";
	CloseTable();
	echo "<br>";
	OpenTable();
	echo "<b>$subject</b><br>";
	echo "<font size=2>"._BY." ";
	if (is_user($user)) {
	    echo "$cookie[1]";
	} else {
	    echo "$anonymous ";
	...
	OpenTable();
	echo "<form action=\"pollcomments.php\" method=post>"
	    ."<font size=\"3\"><B>"._YOURNAME.":</B></FONT> ";
	if (is_user($user)) {
	    echo "<font size=\"2\"><a href=\"user.php\">$cookie[1]</a> <font size=2>[ <a href=\"user.php?op=logout\">"._LOGOUT."</a> ]</font>";
	} else {
	    echo "<font size=\"2\">$anonymous</font>";
	}
	echo "<br><br><font size=\"3\"><B>"._SUBJECT.":</B></FONT><BR>"
		."<INPUT TYPE=\"text\" NAME=\"subject\" SIZE=50 maxlength=85 value=\"$subject\"><br><br>"
		."<P><font size=\"3\"><B>"._UCOMMENT.":</B></FONT><BR>"
		."<TEXTAREA wrap=virtual cols=50 rows=10 name=comment>$comment</TEXTAREA><br>";
		echo"<font size=2>"._ALLOWEDHTML."<br>";
		while (list($key,)= each($AllowableHTML)) echo " &lt;".$key."&gt;";
		echo "<br>";		
...
function CreateTopic ($xanonpost, $subject, $comment, $pid, $pollID, $host_name, $mode, $order, $thold, $posttype) {
	global $user, $userinfo, $EditedMessage, $cookie, $prefix;
	//include("mainfile.php");
...
//begin fake thread control
	list($fake) = mysql_fetch_row(mysql_query("select count(*) from $prefix"._poll_desc." where pollID=$pollID"));
	mysql_query("LOCK TABLES $prefix"._pollcomments." WRITE");
//begin duplicate control
	list($tia) = mysql_fetch_row(mysql_query("select count(*) from $prefix"._pollcomments." where pid='$pid' and pollID='$pollID' and subject='$subject' and comment='$comment'"));
//begin troll control
	if($user) {
		list($troll) = mysql_fetch_row(mysql_query("select count(*) from $prefix"._pollcomments." where (score=-1) and (name='$userinfo[uname]') and (to_days(now()) - to_days(date) < 3)"));
	} elseif(!$score) {
		list($troll) = mysql_fetch_row(mysql_query("select count(*) from $prefix"._pollcomments." where (score=-1) and (host_name='$ip') and (to_days(now()) - to_days(date) < 3)"));
	}
	if((!$tia) && ($fake == 1) && ($troll < 6)) {
		mysql_query("insert into $prefix"._pollcomments." values (NULL, '$pid', '$pollID', now(), '$name', '$email', '$url', '$ip', '$subject', '$comment', '$score', '0')");
	} else {
