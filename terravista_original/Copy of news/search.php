...
switch($op) {
        ...
        default:
                ...
                include("header.php");
		if ($topic>0) {
		    $result = mysql_query("select topicimage, topictext from $prefix"._topics." where topicid=$topic");
		...
		OpenTable();
		if ($type == "users") {
		    echo "<center><font size=\"4\"><b>"._SEARCHUSERS."</b></font></center><br>";
		} elseif ($type == "sections") {
		    echo "<center><font size=\"4\"><b>"._SEARCHSECTIONS."</b></font></center><br>";
		} elseif ($type == "reviews") {
		    echo "<center><font size=\"4\"><b>"._SEARCHREVIEWS."</b></font></center><br>";
		} else {
		    echo "<center><font size=\"4\"><b>"._SEARCHIN." $topictext</b></font></center><br>";
		}
		echo "<table width=\"100%\" border=\"0\"><TR><TD>";
		if (($type == "users") OR ($type == "sections") OR ($type == "reviews")) {
		    echo "<img src=\"$tipath/AllTopics.gif\" align=\"right\" border=\"0\" alt=\"\">";
                } else {
		...
		$catlist = mysql_query("select catid, title from $prefix"._stories."_cat order by title");
		...
		echo "</select>";
		/* Authors Selection */
                $thing = mysql_query("select aid from $prefix"._authors." order by aid");
		...
		$num_sec = mysql_num_rows(mysql_query("select * from $prefix"._sections.""));
		$num_rev = mysql_num_rows(mysql_query("select * from $prefix"._reviews.""));
	...
	if ($type=="stories" OR !$type) {
			...
			list($topictext) = mysql_fetch_row($result2);
				...
				if ($query != "") {
					...
					$match = "$match<br>";
                                ...
                                printf("<tr><td><img src=\"images/links/urlgo.gif\" border=\"0\" alt=\"\">&nbsp;<font size=\"3\"><a href=\"%s\"><b>%s</b></a></font><br><font size=\"2\">"._CONTRIBUTEDBY." $informant<br>"._POSTEDBY." <a href=\"%s\">%s</a>",$furl,$title,$url,$aid,$informant);
		...
		echo "</table>";
                        ...
                        echo "<tr><td><center><font size=\"3\"><b>"._NOMATCHES."</b></font></center><br><br>";
			echo "</td></tr></table>";
                }
		...
		if ($x>=9) {
                ...
                $result = mysql_query("select tid, sid, subject, date, name from $prefix"._comments." where (subject like '%$query%' OR comment like '%$query%') order by date DESC limit $min,$offset");
                $nrows  = mysql_num_rows($result);
                $x=0;
		echo "<br><hr noshade size=\"1\"><center><b>"._SEARCHRESULTS."</b></center><br><br>";
		echo "<table width=\"99%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">\n";
		if ($nrows>0) {
			    ...
			    $datetime = formatTimestamp($date);
                            echo "<tr><td><img src=\"images/links/urlgo.gif\" border=\"0\" alt=\"\">&nbsp;<font size=\"3\"><a href=\"$furl\"><b>$subject</b></a></font><font size=\"2\"><br>"._POSTEDBY." $name"
                        	." "._ON." $datetime<br>"
				.""._ATTACHART.": $title<br>";
			    if ($reply == 1) {
				echo "($reply "._SREPLY.")";
		...
		if ($nrows>0) {
			...
			$pages = count(explode( "<!--pagebreak-->", $text ));
                        echo "<tr><td><img src=\"images/links/urlgo.gif\" border=\"0\" alt=\"\">&nbsp;<font size=\"3\"><a href=\"$furl\"><b>$title</b></a></font><br>"
			    ."<font size=\"2\">"._POSTEDBY." $reviewer<br>"
		...
		if ($x>=9) {
                ...
                $result = mysql_query("select artid, secid, title, content from $prefix"._seccont." where (title like '%$query%' OR content like '%$query%') order by artid DESC limit $min,$offset");
                $nrows  = mysql_num_rows($result);
                $x=0;
		echo "<br><hr noshade size=\"1\"><center><b>"._SEARCHRESULTS."</b></center><br><br>";
		echo "<table width=\"99%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">\n";
		if ($nrows>0) {
			    ...
			    $furl = "sections.php?op=viewarticle&amp;artid=$artid";
                            echo "<tr><td><img src=\"images/links/urlgo.gif\" border=\"0\" alt=\"\">&nbsp;<font size=\"3\"><a href=\"$furl\"><b>$title</b></a></font><font size=\"2\"><br>"._INSECTION.": <a href=\"$surl\">$sectitle</a><br>";
			    if ($pages == 1) {
				echo "($pages "._PAGE.")";
                            } else {
				echo "($pages "._PAGES.")";
		...
		if ($x>=9) {
                ...
                $result = mysql_query("select uid, uname, name from $prefix"._users." where (uname like '%$query%' OR name like '%$query%' OR bio like '%$query%') order by uname ASC limit $min,$offset");
                $nrows  = mysql_num_rows($result);
                $x=0;
		echo "<br><hr noshade size=\"1\"><center><b>"._SEARCHRESULTS."</b></center><br><br>";
		echo "<table width=\"99%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">\n";
		if ($nrows>0) {
                        while(list($uid, $uname, $name) = mysql_fetch_row($result)) {
			    $furl = "user.php?op=userinfo&amp;uname=$uname";
			    if ($name=="") {
				$name = ""._NONAME."";
			    }
                            echo "<tr><td><img src=\"images/links/urlgo.gif\" border=\"0\" alt=\"\">&nbsp;<font size=\"3\"><a href=\"$furl\"><b>$uname</b></a></font><font size=\"2\"> ($name)";
			    if (is_admin($admin)) {
